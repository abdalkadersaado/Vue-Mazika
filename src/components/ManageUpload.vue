<template>
  <div class="bg-white rounded border border-gray-200 relative flex flex-col">
    ...
    <div class="p-6">
      ...
      <input type="file" name="" multiple id="" @change.prevent="upload($event)" />
      <hr class="my-6" />
      <!-- Progess Bars -->
  ...
  methods: {
    upload($event) {
      ...
      files.forEach(async (file) => {
        ...
        const task = createChildStorageRef(`songs/${file.name}`, file);
        ...
        task.on(
          ...
          async () => {
            const song = {
              ...
              display_name: checkCurrentUser().displayName,
            ...
            song.url = await downloadUrl(task);

            const songRef = await createSongDocument(song);
            const songSnapshot = await customSnapshot(songRef);
